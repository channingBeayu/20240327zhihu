...
class EventsExtraction:
            ...
            if ress:
                for res in ress:
                    ...
                    if pre_part == '' or post_part == '':
                        continue
                    data = {'pre_wd': res[0], 'pre_part': pre_part,
                            'post_wd': res[2], 'post_part': post_part}
    ...
    def extract_main(self, content):
        ...
        for sent in sents:
            ...
            if 'type' in data:
                if data['tuples']['pre_wd']:
                    data['tag'] = data['tuples']['pre_wd'] + '-' + data['tuples']['post_wd']
                else:
                    data['tag'] = data['tuples']['post_wd']
                datas.append(data)
        return datas
