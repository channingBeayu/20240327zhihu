...
class NodeMerge:
    ...
    def to_graph(self, events, event_infos):
        ...
        while i < len(events):
            ...
            docs = [events[i], events[i+1]]
            embeddings = self.embedding_model.embed_documents(document=docs)
            res = self.topic_model.transform(docs, embeddings)[0]
        ...
        #     post_topic = res[1]
        #
        #     if events[i] not in node_dict:
        #         node_dict[events[i]] = {'k', pre_topic}
        #     if events[i+1] not in node_dict:
        #         node_dict[events[i+1]] = {'k', post_topic}
        #
        #     edges.append({'from': events[i], 'to': events[i+1],
        #                   'from_k': pre_topic, 'to_k': post_topic,
        #                   'label': event_infos[i]['label']})
        #     i = i+2
