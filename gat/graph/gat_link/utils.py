...
def load_data(path="../dataset/mydata/", dataset="zh"):
    ...
    features = embedding_model.embed_documents(document=idx_features_labels[1])
...
def load_data_cora(path="../dataset/cora/", dataset="cora"):
    """Load citation network dataset (cora only for now)"""
    print('Loading {} dataset...'.format(dataset))
    idx_features_labels = pd.read_csv("{}{}.content".format(path, dataset), sep='\t', header=None)
