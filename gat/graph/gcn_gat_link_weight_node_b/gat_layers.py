...
class GraphAttentionLayer(nn.Module):
    ...
    def forward(self, h, adj, link_weight):  # h为节点特征，adj为邻接矩阵
        ...
        e = self._prepare_attentional_mechanism_input(Wh, link_weight)
        # e = e * link_weight

        # 注意力系数可能为0，这里需要进行筛选操作，便于后续乘法
        zero_vec = -9e15 * torch.ones_like(e)
        attention = torch.where(adj > 0, e, zero_vec)  # adj > 0表示，只有当两个节点直接连接时，它们之间的注意力系数才非零
        attention = F.softmax(attention, dim=1)
        attention = F.dropout(attention, self.dropout, training=self.training)
        
        # 输出结合注意力系数的特征矩阵
        # (N, N) * (N, out_dim)  (2708, 2708) * (2708, 8)
        # (10, 10) * (10, 8)  = (10, 8)
    ...
    def _prepare_attentional_mechanism_input(self, Wh, link_weight):
        # Wh.shape (N, out_feature)
        # self.a.shape (2 * out_feature, 1)
        # Wh1&2.shape (N, 1)
        # e.shape (N, N)
        Wh1 = torch.matmul(Wh, self.a[:self.out_features, :])
        # todo: 在注意力机制中，通常会使用两个权重矩阵来计算注意力系数，一个用于计算正向的注意力系数，另一个用于计算反向的注意力系数。
        Wh2 = torch.matmul(Wh, self.a[self.out_features:, :])
        # broadcast add
        e = Wh1 + Wh2.T  # 先把Wh2.T(1, 2708) 变为2708行相同的，再分别加Wh1(2708,1)的一个数


        e2 = torch.matmul(link_weight, self.b)
        e = e+e2
